...
Serial.begin(115200);  // 加上這行，初始化串口
...
//byte TCCR0B_=TCCR0B;//save settings delay-Timer0 if needed
  ...
  byte     gain=2;   //adjust Loudness 1-6
  ...
  while(1) { // Read 24Bit-Mic-Data                            Sampling 25Khz
    ...
    // Serial.write(0xFF); Serial.write(0xFF);  // sync 標誌
    Serial.write(sample & 0xFF);
    Serial.write((sample >> 8) & 0xFF);
    // Serial.write((uint8_t)(sample & 0xFF));        // LSB
    // Serial.write((uint8_t)((sample >> 8) & 0xFF)); // MSB
